...
class face {
      ...
      public:
	          ...
	          int m,n;
              dirn arbitf();
              int read(int faceid,int size,float tconductivity1,float spheat1,float density1);
...
int face::read(int faceid,int size,float tconductivity1,float spheat1,float density1)
{
	...
	if(vf2[i].value == 1)
	{
		...
		if(f[faceid1].normal.x == 1 ||f[faceid1].normal.x == -1)
		{
			...
			if(b==0)
			{
				...
				{
					x[1] =	eprop[sid1].ecentre.z - eprop[sid1].bth/2;
			        x[2] = eprop[sid1].ecentre.z + eprop[sid1].bth/2;
					y[1] = eprop[sid1].ecentre.y-eprop[sid1].lth/2 - eprop[sid2].ecentre.y-eprop[sid2].thickness/2;
					y[2] = eprop[sid1].ecentre.y+eprop[sid1].lth/2 - eprop[sid2].ecentre.y-eprop[sid2].thickness/2;
					eta[1]=eprop[sid2].ecentre.z-eprop[sid2].lth/2;
			        eta[2]=eprop[sid2].ecentre.z+eprop[sid2].lth/2;
					psi[1]=eprop[sid2].ecentre.x-eprop[sid2].bth/2 - eprop[sid1].ecentre.x-eprop[sid1].thickness/2;
					psi[2]=eprop[sid2].ecentre.x+eprop[sid2].bth/2 - eprop[sid1].ecentre.x-eprop[sid1].thickness/2;
				}
			
				else
				{
					x[1] = f[faceid1].startpt.z - f[faceid2].startpt.z;
					x[2] = f[faceid1].stptbth.z - f[faceid2].startpt.z;
					y[1] = f[faceid1].startpt.y;
					y[2] = f[faceid1].stptlth.y;
					eta[1]=f[faceid2].startpt.x - f[faceid1].startpt.x;
					eta[2]=f[faceid2].stptlth.x - f[faceid1].startpt.x;
					psi[1]=f[faceid2].startpt.y;
					psi[2]=f[faceid2].stptbth.y;
				};   
			};
		};
		...
		if(f[faceid1].normal.y == 1 ||f[faceid1].normal.y == -1)
		{
			...
			if(b==0)
			{
				...
				{                                                                                 //I THINK I MIGHT NEED TO INTERCHANGE X[] N Y[] @ FEW LHS VALUES
					x[1] = f[faceid1].startpt.x - f[faceid2].startpt.x;
					x[2] = f[faceid1].endpt.x - f[faceid2].startpt.x;
					y[1] = f[faceid1].startpt.z;
			        y[2] = f[faceid1].endpt.z;
					eta[1]=f[faceid2].startpt.y- f[faceid1].startpt.y;
			        eta[2]=f[faceid2].stptlth.y- f[faceid1].startpt.y;
					psi[1]=f[faceid2].startpt.z;
					psi[2]=f[faceid2].stptbth.z;
				}
				
				else
				{
                    x[1] = f[faceid1].startpt.x;
					x[2] = f[faceid1].stptlth.x;
					y[1] = f[faceid1].startpt.y - f[faceid2].startpt.y;
					y[2] = f[faceid1].stptbth.y - f[faceid2].startpt.y;
					eta[1]=f[faceid2].startpt.z - f[faceid1].startpt.z;
					eta[2]=f[faceid2].stptlth.z - f[faceid1].startpt.z;
					psi[1]=f[faceid2].startpt.x;
					psi[2]=f[faceid2].stptbth.x;
				};   
			};
		};
		...
		cout<<"View factor computed"<<endl;
		return sum;
	};
...
void numerical(elementprop *eprop,face *f,int fa,int noof_elements)
{
	...
	int k,i,j,m,p,timestep=1,n,nfaceid1,nfaceid,klthnve,klthpve,kbthnve,kbthpve,kthicknessnve,transient=0,kthicknesspve,count1,diffcount=9;
					...
					if(eprop[k].eload.internalradiation.i)
					{
						...
						for(m=0;m<noof_elements;m++)
						{
							if(eprop[m].eload.internalradiation.i)
							{
								...
								cout<<"Computing view factors"<<endl;
							};
						};
